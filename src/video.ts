...
export function calculateGeometry(): void {
   ...
   if(video.border_h       !== undefined && (video.border_h      > 40 || video.border_h      < 0)) video.border_h      = undefined;

   BORDER_V        = (video.border_top    !== undefined ? video.border_top    : 17);
   BORDER_V_BOTTOM = (video.border_bottom !== undefined ? video.border_bottom : 23);   
   HIDDEN_SCANLINES_TOP    = 65 - BORDER_V; 
   HIDDEN_SCANLINES_BOTTOM = 56 - BORDER_V_BOTTOM;   
   BORDER_H = video.border_h !== undefined ? video.border_h : 40;    
...
export function drawFrame_y()
{
   ...
   if(raster_y >= SCREEN_H) {
      ...
      laser500.cpu.interrupt(false, 0); // generate VDC interrupt
   }
}
...
function drawFrame_y_border(y) 
{
   ...
   for(let x=0; x<SCREEN_W; x++) {
      ...
      if(!inside)
      {
         setPixelBorder(x,y, laser500.vdc_border_color);
      }
   }
}
...
{  
   let video = laser500.vdc_page_7 ? laser500.bank7 : laser500.bank3;

   if(laser500.vdc_graphic_mode_enabled) 
   {
      let offs;
      switch(laser500.vdc_graphic_mode_number) {            
         case 5: // GR 5 640x192 1bpp            
            offs = offs_2[y];
            for(let x=0; x<80; x++, offs++) {
               const row = video[offs]; 
               for(let xx=0;xx<8;xx++) {                     
                  const pixel_color = (row & (1<<xx)) > 0 ? laser500.vdc_text80_foreground : laser500.vdc_text80_background;
                  setPixel640(x*8+xx, y, pixel_color);
               }
            }
            break;
   ...
   // text modes 
   else if(laser500.vdc_text80_enabled)
   {      
      // 80 columns text mode          
      const by = y >> 3;
      const oy = y & 0b111;
      ...
      for(let x=0; x<80; x++, offs++)
      {
         ...
         const row  = charset[laser500.charset_offset+startchar+oy];
         for(let xx=0;xx<8;xx++) {
            const pixel_color = (row & (1<<xx)) > 0 ? laser500.vdc_text80_foreground : laser500.vdc_text80_background;
            setPixel640(x*8+xx, y, pixel_color);
         }
...
function setPixelBorder(x: number, y: number, color: number) {  
   if(DOUBLE_SCANLINES) {    
...
function setPixel640(x: number, y: number, color: number) {
   if(DOUBLE_SCANLINES) {    
      const c0 = palette[color];   
      const c1 = halfpalette[color];   
      const xx = x + BORDER_H;
...
function setPixel320(x: number, y: number, color: number) {   
   if(DOUBLE_SCANLINES) {    
      const c0 = palette[color];   
      const c1 = halfpalette[color];   
      const yy = (y + BORDER_V) * 2;
...
function setPixel160(x: number, y: number, color: number) {   
   if(DOUBLE_SCANLINES) {    
      const c0 = palette[color];   
      const c1 = halfpalette[color];   
